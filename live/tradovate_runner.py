"""Tradovate live runner: bridges Tradovate bar data to Python strategy engine.

Mirrors the bar-by-bar loop from the backtest engine and ib_runner.py but
reads bars from the Tradovate REST/WebSocket API.

Usage:
    python3 live/tradovate_runner.py --env demo --contracts 4
    python3 live/tradovate_runner.py --env live --contracts 4  # live trading

Setup:
    1. Create a Tradovate demo account at https://trader.tradovate.com
    2. Get API credentials (appId, cid, sec) from https://trader.tradovate.com/app
    3. Copy live/.env.example to live/.env and fill in credentials
    4. python3 -m pip install websocket-client python-dotenv requests
    5. Run this script: python3 live/tradovate_runner.py
"""
...
from live.tradovate_bridge import TradovateBridge, TradovateConfig
...
# ── Production parameters (validated via backtest + optuna) ──────────
...
class TradovateRunner:
    """Main live runner: bridges Tradovate data to Python strategy engine.

    Polls Tradovate for new 1-minute bars, runs the full strategy pipeline,
    and executes bracket orders via the Tradovate API.
    """

    def __init__(
        self,
        tv_config: TradovateConfig = TradovateConfig(),
        ...
        self.bridge = TradovateBridge(tv_config)
        ...
        self._trade_id: Optional[int] = None  # Tradovate order ID
    ...
    def run(self) -> None:
        ...
        logger.info("=" * 60)
        logger.info("MANCINI TRADOVATE RUNNER STARTING")
        logger.info(f"  Symbol: {self.bridge.config.symbol}")
        logger.info(f"  Env:    {self.bridge.config.env}")
        logger.info("=" * 60)
        ...
        logger.info("Waiting for bars from Tradovate...")
        try:
            while self._running:
                ...
                # Sync position with Tradovate periodically
                self._sync_position()
    ...
    def stop(self) -> None:
        ...
        self.bridge.disconnect()
        logger.info("Tradovate Runner stopped")

    # ── Session initialization ───────────────────────────────────────

    def _initialize_session(self) -> bool:
        """Initialize strategy from Tradovate historical bars.
        ...
        """
        logger.info("Initializing session from Tradovate data...")
    ...
    def _process_bar(self, bar: dict) -> None:
        """Process a new bar from Tradovate.
    ...
    def _evaluate_and_enter(self, signal: Signal, current_time: time, timestamp: datetime) -> None:
        ...
        # Execute via Tradovate -- bracket order (market + SL + TP as OSO)
        ...
        if trade_id is None:
            logger.error("Entry order rejected by Tradovate")
            return
    ...
    def _handle_exit_action(self, action: ExitAction, timestamp: datetime) -> None:
        """Translate ExitAction from ExitManager into Tradovate orders."""
    ...
    def _sync_position(self) -> None:
        """Sync local position state with Tradovate actual position.

        Handles cases where Tradovate's bracket order (stop/target) fires
        independently of Python's exit logic.
        """
        if self._position is None or not self._position.is_open:
            return

        tv_pos = self.bridge.get_position()
        if tv_pos is None:
            # Position closed on Tradovate side (bracket order filled)
            logger.info("Position closed on Tradovate side (bracket order filled)")
            self._position.remaining_contracts = 0
            self._position.phase = ExitPhase.CLOSED
            self.position_manager.close_position(
                exit_price=0,  # Unknown exact fill price
                timestamp=datetime.now(),
                exit_reason="Tradovate bracket fill",
                pattern_type=self._pattern_type,
            )
            self._position = None
            self._trade_id = None
    ...
    def _recover_position(self, pos_data: dict) -> None:
        """Reconstruct TradePosition from Tradovate position on restart."""
        ...
        logger.warning(f"Recovered position: {qty} @ {entry:.2f}, "
...
def main():
    """Run the Tradovate bridge with production params."""
    import argparse

    parser = argparse.ArgumentParser(description="Mancini Tradovate Runner")
    parser.add_argument("--env", default="demo", choices=["demo", "live"],
                        help="Tradovate environment (default: demo)")
    parser.add_argument("--contracts", type=int, default=4,
                        help="Number of contracts to trade")
    parser.add_argument("--symbol", default="MES",
                        help="Symbol (default: MES)")
